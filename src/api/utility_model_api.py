   ...
   try:
       ...
       async with httpx.AsyncClient() as client:
           response = await client.get(
               settings.UTILITY_MODEL_API_URL,
               params=params,
               timeout=30.0
           )
           ...
           try:
               ...
               if not data.get('response', {}).get('body', {}).get('items'):
                   raise HTTPException(
                       status_code=404,
                       detail=f"등록번호 {registration_number}에 대한 실용신안 데이터가 없습니다."
                   )
           ...
           with db.get_connection() as conn:
               ...
               try:
                   ...
                   # 4. 발명자 정보 저장
                   inventors = items.get('utilityModelOwner', [])  # 실용신안의 경우 발명자 키가 다름
                   for inventor in inventors:
                       ...
                       ''', (
                           ip_right_id,
                           'inventor',
                           inventor.get('utilityModelOwnerName'),
                           inventor.get('utilityModelOwnerNatl'),
                           inventor.get('utilityModelOwnerAddr'),
                           inventor.get('utilityModelOwnerCd'),
                           None  # 실용신안에는 일련번호가 없음
                       ))
                   
                   # 5. 권리자 정보 저장
                   owners = items.get('right', [])
                   for owner in owners:
                       cursor.execute('''
                       ...
                       ''', (
                           ip_right_id,
                           owner.get('rgstCsName'),
                           None,  # 실용신안에는 영문이름이 없음
                           None,  # 실용신안에는 국적이 별도로 없음
                           None,  # 실용신안에는 주소가 별도로 없음
                           owner.get('rgstCsName'),
                           owner.get('rgstCsDate'),
                           owner.get('rgstCsReason'),
                   ...
                   conn.commit()
                   logger.info(f"실용신안 데이터 저장 완료: {registration_number}")

...
async def fetch_design_data(registration_number: str, service_key: str):
   """디자인 등록원부 데이터를 조회하고 저장하는 함수"""
   try:
       params = {
       ...
       async with httpx.AsyncClient() as client:
           ...
           try:
               ...
               if not data.get('response', {}).get('body', {}).get('items'):
                   raise HTTPException(
                       status_code=404,
                       detail=f"등록번호 {registration_number}에 대한 디자인 데이터가 없습니다."
                   )
               ...
               try:
                   ...
                   cursor.execute('''
                       INSERT OR REPLACE INTO ip_rights (
                           rgst_no, right_type, appl_no, title,
                   ...
                   ''', (
                       ...
                       'design',
                       items.get('applNo'),
                       items.get('dsTgtNm'),  # 디자인의 경우 물품명이 타이틀
                       items.get('applDate'),
                       items.get('rgstDate'),
                       items.get('pubNo'),
                       items.get('pubDate'),
                   ...
                   cursor.execute('''
                       INSERT INTO design_details (
                           ip_right_id, ds_tgt_nm, ptdsn_yn, ds_cls_nm,
                           ds_main_cs_cd, ds_lc_cs_cd, rtact_date
                       ) VALUES (?, ?, ?, ?, ?, ?, ?)
                   ''', (
                       ip_right_id,
                       items.get('dsTgtNm'),
                   ...
                   for creator in creators:
                       ...
                       ''', (
                           ip_right_id,
                           'originator',
                           creator.get('originatorName'),
                           creator.get('originatorNatl'),
                           creator.get('originatorAddr'),
                           creator.get('originatorCd'),
                           creator.get('originatorSeq')
                       ))
                   ...
                   conn.commit()
                   logger.info(f"디자인 데이터 저장 완료: {registration_number}")
